(function() {
	...
	var $list, $global_stats, global_stats = { "series-count": 0, "episode-count": 0, "watched-episodes-count": 0 };
	...
	function draw_series(data) {
		$.each(data, function(index, series) {
			var $series = $("<section />").addClass("series");
			var stats = { "episode-count": 0, "watched-episodes-count": 0 };

			// series title
			$("<h1 />").text(series["series-title"]).appendTo($series);
			var $series_stats = $("<div />").addClass("stats").appendTo($series);
